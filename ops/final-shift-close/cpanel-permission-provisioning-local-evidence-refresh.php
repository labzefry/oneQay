...
// Author by Lab | zefry
// Read-only refresh of a previously signed cPanel-local permission provisioning
// evidence envelope after canonical-main verifier compatibility changes.
// No DDL/DML. No permission mutation. No credential output.
...
const ONEQAY_REFRESH_SCHEMA_VERSION = 1;
const ONEQAY_REFRESH_EVIDENCE_TYPE = 'CPANEL_LOCAL_PERMISSION_PROVISIONING';
const ONEQAY_REFRESH_TTL_SECONDS = 900;
const ONEQAY_REFRESH_SIGNING_CONTEXT = 'oneqay-permission-cpanel-local-provisioning-v1';
const ONEQAY_REFRESH_PERMISSION = 'pos.shift.close';
const ONEQAY_REFRESH_ATTESTATION = 'LOCAL_PERMISSION_PROVISIONING_AND_POST_VERIFICATION';
...
try {
    ...
    $signingKey = hash_hmac('sha256', ONEQAY_REFRESH_SIGNING_CONTEXT, $password, true);
    ...
    $priorExpected = [
        'schema_version' => ONEQAY_REFRESH_SCHEMA_VERSION,
        'evidence_type' => ONEQAY_REFRESH_EVIDENCE_TYPE,
        ...
        'role_id' => $roleId,
        'permission_id' => ONEQAY_REFRESH_PERMISSION,
        ...
        'attestation_mode' => ONEQAY_REFRESH_ATTESTATION,
    ...
    if (! is_int($priorIssued)
        ...
        || ($priorExpires - $priorIssued) > ONEQAY_REFRESH_TTL_SECONDS
    ) {
    ...
    if (! hash_equals(ONEQAY_REFRESH_PERMISSION, $permission->value())) {
    ...
    if (! $tenantControl) {
        throw new RuntimeException('Control actor no longer has tenant-scoped protected authority.');
    }
    ...
    $payload = [
        ...
        'expires_at_unix' => $issued + ONEQAY_REFRESH_TTL_SECONDS,
    ...
    if (strlen($encoded) > 32768) {
        throw new RuntimeException('Refreshed evidence oversized.');
    }

    writePrivate($output, $encoded."\n");

    fwrite(STDOUT, "RESULT=SUCCESS\n");
    fwrite(STDOUT, "MODE=CPANEL_LOCAL_PERMISSION_EVIDENCE_REFRESH\n");
    ...
    fwrite(STDOUT, "EXPIRES_AT_UNIX=".($issued + ONEQAY_REFRESH_TTL_SECONDS)."\n");
    exit(0);
} catch (Throwable) {
    fwrite(STDERR, "RESULT=FAILED\n");
    fwrite(STDERR, "FAILED_STAGE={$stage}\n");
    fwrite(STDERR, "ERROR=LOCAL_PERMISSION_EVIDENCE_REFRESH_FAILED\n");
    exit(1);
}
