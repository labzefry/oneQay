...
return new class extends Migration
{
    ...
    {
        Schema::create('oneqay_pos_shift_closing_cash_evidence', function (Blueprint $table): void {
            ...
            $table->unsignedBigInteger('closing_cash_atomic');
            ...
            $table->primary(['tenant_id', 'evidence_id'], 'pk_pos_shift_close_cash');
            $table->unique(['tenant_id', 'operation_id'], 'uq_pos_shift_close_cash_operation');
            $table->unique(['tenant_id', 'shift_id'], 'uq_pos_shift_close_cash_shift');
            $table->index(
                ['tenant_id', 'outlet_id', 'recorded_at_unix'],
                'ix_pos_shift_close_cash_outlet_time',
            );

            $table->foreign(['tenant_id', 'shift_id'], 'fk_pos_shift_close_cash_shift')
                ->references(['tenant_id', 'shift_id'])->on('oneqay_pos_shifts')
                ->restrictOnDelete()->restrictOnUpdate();
            $table->foreign(['tenant_id', 'opening_cash_evidence_id'], 'fk_pos_shift_close_cash_opening')
                ->references(['tenant_id', 'evidence_id'])->on('oneqay_pos_shift_opening_cash_evidence')
                ->restrictOnDelete()->restrictOnUpdate();
            $table->foreign(['tenant_id', 'actor_identity_id'], 'fk_pos_shift_close_cash_actor')
                ->references(['tenant_id', 'id'])->on('oneqay_identities')
                ->restrictOnDelete()->restrictOnUpdate();
            $table->foreign(['tenant_id', 'organization_id'], 'fk_pos_shift_close_cash_org')
                ->references(['tenant_id', 'id'])->on('oneqay_organizations')
                ->restrictOnDelete()->restrictOnUpdate();
            $table->foreign(['tenant_id', 'outlet_id'], 'fk_pos_shift_close_cash_outlet')
                ->references(['tenant_id', 'id'])->on('oneqay_outlets')
                ->restrictOnDelete()->restrictOnUpdate();
            $table->foreign(['tenant_id', 'device_id'], 'fk_pos_shift_close_cash_device')
