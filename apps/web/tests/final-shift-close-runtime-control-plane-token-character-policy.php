...
if ($mode === 'materialization-invalid-expected') {
    ...
    try {
        ...
        $assert($exception->getStatusCode() === 503, 'CHAR-003 materialization invalid expected token remains HTTP 503');
        fwrite(STDOUT, "Final Shift Close materialization invalid expected token disposition: OK\n");
        exit(0);
    }

    throw new RuntimeException('CHAR-003 materialization invalid expected token was not rejected.');
}
...
if ($mode === 'db-invalid-expected') {
    ...
    $assert($response->getStatusCode() === 404, 'CHAR-004 DB invalid expected token remains cloaked HTTP 404');
...
if ($mode === 'materialization-invalid-bearer') {
    ...
    try {
        ...
        $assert($exception->getStatusCode() === 401, 'CHAR-005 materialization invalid bearer remains HTTP 401');
        fwrite(STDOUT, "Final Shift Close materialization invalid bearer disposition: OK\n");
        exit(0);
    }

    throw new RuntimeException('CHAR-005 materialization invalid bearer was not rejected.');
}
...
if ($mode === 'db-invalid-bearer') {
    ...
    $response = $middleware->handle($request, static fn () => response('', 204));
    $assert($response->getStatusCode() === 404, 'CHAR-006 DB invalid bearer remains cloaked HTTP 404');
...
$assert($bootstrapResponse->getStatusCode() === 200, 'CHAR-007 application bootstrap remains healthy');
