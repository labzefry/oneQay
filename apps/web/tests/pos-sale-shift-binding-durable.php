...
$assert=static function(bool $ok,string $case):void {
    if(!$ok){throw new RuntimeException('Sprint57 sale-to-shift binding regression failed: '.$case);}
};

$dir=sys_get_temp_dir().DIRECTORY_SEPARATOR.'oneqay-s57-binding-'.getmypid();
@mkdir($dir,0700,true);
$db=$dir.DIRECTORY_SEPARATOR.'pos.sqlite';
$assert(touch($db),'sqlite create');

$app['config']->set('database.default','s57-binding');
$app['config']->set('database.connections.s57-binding',[
...
$manager=$app->make('db');
$manager->purge('s57-binding');
$manager->setDefaultConnection('s57-binding');
$connection=$manager->connection('s57-binding');
$connection->getPdo();
...
foreach([
    ...
    ['tenant-alpha','cashier-alpha','organization-other'],
    ['tenant-beta','cashier-beta','organization-beta'],
] as [$tenant,$id,$org]){
...
$connection->table('oneqay_pos_sale_catalog_items')->insert([
    ...
    'available_quantity'=>20,'active'=>true,
]);
...
$parameters=(new ReflectionClass(SaleCommand::class))->getConstructor()?->getParameters()??[];
foreach($parameters as $parameter){
    $assert(!in_array(strtolower($parameter->getName()),['shift','shiftid','shift_id'],true),'caller shift parameter');
}
...
$insertShift([
    'tenant_id'=>'tenant-alpha','shift_id'=>str_repeat('f',32),'operation_id'=>'shift-wrong-org',
    'actor_identity_id'=>'cashier-alpha','organization_id'=>'organization-other',
    'outlet_id'=>'outlet-alpha','device_id'=>'device-alpha','correlation_id'=>'corr-wrong-org',
]);
$deny('wrong-organization');
$connection->table('oneqay_pos_shifts')->where('operation_id','shift-wrong-org')->update(['active_slot'=>null]);

$exactShiftId=str_repeat('e',32);
$insertShift([
    'tenant_id'=>'tenant-alpha','shift_id'=>$exactShiftId,'operation_id'=>'shift-exact',
    'actor_identity_id'=>'cashier-alpha','organization_id'=>'organization-alpha',
    'outlet_id'=>'outlet-alpha','device_id'=>'device-alpha','correlation_id'=>'corr-exact',
]);

$command=$make('sale-bound',2);
$receipt=$sales->complete($command);
$sale=$connection->table('oneqay_pos_sales')->where('sale_id',$receipt->saleId())->first();
$assert($sale!==null,'sale persisted');
$assert(is_string($sale->shift_id)&&hash_equals($exactShiftId,$sale->shift_id),'exact shift persisted');
$assert((int)$connection->table('oneqay_pos_sale_catalog_items')->where('product_id','product-a')->value('available_quantity')===18,'stock decrement');

$connection->table('oneqay_pos_shifts')->where('operation_id','shift-exact')->update(['active_slot'=>null]);
$nextShiftId=str_repeat('9',32);
$insertShift([
    'tenant_id'=>'tenant-alpha','shift_id'=>$nextShiftId,'operation_id'=>'shift-next',
    'actor_identity_id'=>'cashier-alpha','organization_id'=>'organization-alpha',
    'outlet_id'=>'outlet-alpha','device_id'=>'device-alpha','correlation_id'=>'corr-next',
]);
$replay=$sales->complete($command);
$assert($replay->saleId()===$receipt->saleId(),'replay identity');
$assert($connection->table('oneqay_pos_sales')->where('sale_id',$receipt->saleId())->value('shift_id')===$exactShiftId,'replay preserved original shift');
$assert((int)$connection->table('oneqay_pos_sale_catalog_items')->where('product_id','product-a')->value('available_quantity')===18,'replay stock');

try{
    $connection->table('oneqay_pos_sales')->where('sale_id',$receipt->saleId())->update(['organization_id'=>'organization-other']);
    $assert(false,'database accepted mismatched shift context');
}catch(Throwable){}
$assert($connection->table('oneqay_pos_sales')->where('sale_id',$receipt->saleId())->value('organization_id')==='organization-alpha','database mismatch rollback');

$connection->table('oneqay_pos_sales')->where('sale_id',$receipt->saleId())->update(['shift_id'=>null]);
$assert($connection->table('oneqay_pos_sales')->where('sale_id',$receipt->saleId())->value('shift_id')===null,'historical null simulation');
$legacyReplay=$sales->complete($command);
$assert($legacyReplay->saleId()===$receipt->saleId(),'historical null replay identity');
$assert($connection->table('oneqay_pos_sales')->where('sale_id',$receipt->saleId())->value('shift_id')===null,'historical null replay not repaired');

try{$sales->complete($make('sale-bound',1));$assert(false,'conflicting replay accepted');}catch(PosTransactionViolation){}

$manager->disconnect('s57-binding');
$manager->purge('s57-binding');
@unlink($db);
@rmdir($dir);

echo "Sprint57 JRN-010 immutable sale-to-shift binding regression passed.\n";
