...
$assert = static function (bool $condition, string $message): void {
    if (! $condition) {
        throw new RuntimeException('Sprint196 regression failed: '.$message);
    }
};

$root = sys_get_temp_dir().DIRECTORY_SEPARATOR.'oneqay-s196-'.bin2hex(random_bytes(8));
...
$releaseId = 'm75-preview-196abc196abc';
$now = 1790006000;
$installationToken = 's196-install.'.str_repeat('H', 48);
$approvalToken = 's196-approve.'.str_repeat('J', 48);
$technicalPreviewApprovalToken = 's196-preview.'.str_repeat('K', 48);
...
$writeJson = static function (string $path, array $payload): void {
    ...
    if (file_put_contents($path, $encoded, LOCK_EX) === false) {
        throw new RuntimeException('Unable to write Sprint196 JSON fixture.');
    }
    @chmod($path, 0600);
};
...
try {
    ...
    foreach ([
        ...
        if (! is_dir($directory) && ! mkdir($directory, 0700, true) && ! is_dir($directory)) {
            throw new RuntimeException('Unable to create Sprint196 application fixture.');
        }
    }
    ...
    foreach ($fixtureCopies as $sourcePath => $targetPath) {
        if (! copy($sourcePath, $targetPath)) {
            throw new RuntimeException('Unable to copy Sprint196 target preflight source fixture.');
        }
    }
    ...
    if (file_put_contents($releaseRoot.DIRECTORY_SEPARATOR.'RELEASE.json', $releaseJson, LOCK_EX) === false) {
        throw new RuntimeException('Unable to write Sprint196 release fixture.');
    }
    ...
    fwrite(STDOUT, "Sprint196 Technical Preview target environment preflight regression passed.\n");
} finally {
    $remove($root);
}
