...
final readonly class FinalShiftCloseDurableRuntimeDependencyEnvelopeEvidenceProducer
{
    ...
    public function violations(
        ...
        foreach (self::REQUIRED_COMPONENTS as $component => $expectedPath) {
            ...
            foreach (['environment_id', 'runtime_class', 'exact_running_source_commit', 'exact_running_artifact_sha256'] as $field) {
    ...
    /** @param array<string, mixed> $selection @param array<string, mixed> $attestation @param array<string, mixed> $capabilityEvidence @param array<string, mixed> $observations */
    public function canProduce(array $selection, array $attestation, array $capabilityEvidence, array $observations): bool
    {
     ...
     */
    public function produce(array $selection, array $attestation, array $capabilityEvidence, array $observations): array
    {
        ...
        $capabilityQualification = $this->capabilityQualifier->qualify($selection, $attestation, $capabilityEvidence);
        $evidence = $this->buildEvidence($selection, $capabilityQualification, $observations);
        $qualification = $this->dependencyQualifier->qualify($selection, $attestation, $capabilityEvidence, $evidence);
    ...
    /** @param array<string, mixed> $selection @param array<string, mixed> $capabilityQualification @param array<string, mixed> $observations @return array<string, mixed> */
    private function buildEvidence(array $selection, array $capabilityQualification, array $observations): array
    {
        ...
        foreach (self::REQUIRED_COMPONENTS as $component => $sourcePath) {
            $record = is_array($observations['components'][$component] ?? null) ? $observations['components'][$component] : [];
            $identity = [
                'feature' => self::FEATURE,
                'component' => $component,
                'state' => 'VERIFIED',
