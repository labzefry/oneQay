...
final readonly class LaravelFirstControlPrincipalCredentialBootstrapRepository implements FirstControlPrincipalCredentialBootstrapRepository
{
    ...
    public function bootstrap(
        ...
        try {
            ...
            $target = $this->resolveInitialControlPrincipal($tenant);

            if (! $this->identityExists($tenant, $target)
                || ! $this->protectedRoleStateCompatible($tenant)
                || ! $this->targetStillOwnsProtectedControlAssignment($tenant, $target)) {
                $this->ineligible();
            }
            ...
            if ($this->activeInitialPasswordEnrollmentExists($tenant, $target)) {
                $this->fail(
