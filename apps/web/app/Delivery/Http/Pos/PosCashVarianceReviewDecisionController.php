...
// Author by Lab | zefry
final class PosCashVarianceReviewDecisionController
{
    ...
    public function __invoke(Request $request): JsonResponse
    {
        ...
        try {
            $result = $this->reconciliation->recordReview(
                $this->requiredString($request->input('closing_evidence_id')),
                $this->requiredString($request->input('operation_id')),
                $this->requiredString($request->input('explanation_evidence_id')),
                $this->requiredString($request->input('review_outcome')),
                $correlationId,
            );

            return response()->json([
                'review_evidence_id' => $result->reviewEvidenceId(),
                'operation_id' => $result->operationId(),
                'closing_evidence_id' => $result->closingCashEvidenceId(),
                'explanation_evidence_id' => $result->cashVarianceExplanationEvidenceId(),
                'reviewer_actor_identity_id' => $result->reviewerActorIdentityId(),
                'review_outcome' => $result->reviewOutcome(),
                'reviewed_at_unix' => $result->reviewedAtUnix(),
                'refresh_required' => true,
                'correlation_id' => $result->correlationId(),
            ], 200, ['Cache-Control' => 'no-store, private']);
        } catch (InvalidArgumentException) {
            return response()->json(
                SafeErrorEnvelope::make('POS_CASH_VARIANCE_REVIEW_INVALID', $correlationId),
                422,
                ['Cache-Control' => 'no-store, private'],
            );
        } catch (DurableAuthorizationViolation|PosAccessViolation) {
            return response()->json(
                SafeErrorEnvelope::make('POS_CASH_VARIANCE_REVIEW_AUTHORIZATION_DENIED', $correlationId),
                403,
                ['Cache-Control' => 'no-store, private'],
            );
        } catch (PosTransactionViolation) {
            return response()->json(
                SafeErrorEnvelope::make('POS_CASH_VARIANCE_REVIEW_CONFLICT', $correlationId),
                409,
                ['Cache-Control' => 'no-store, private'],
            );
        }
