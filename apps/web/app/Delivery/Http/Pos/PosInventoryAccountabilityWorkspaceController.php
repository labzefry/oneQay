...
// Author by Lab | zefry
final class PosInventoryAccountabilityWorkspaceController
{
    public function __construct(private readonly ViewPosInventoryAccountabilityWorkspace $workspace) {}
    ...
    public function __invoke(Request $request): Response|JsonResponse
    {
        ...
        try {
            ...
            return Inertia::render('Pos/InventoryAccountability', [
                'scope' => [
                ...
                'recent_movements' => $snapshot->recentMovements(),
                'correlation_id' => $correlationId,
            ]);
        } catch (DurableAuthorizationViolation|PosAccessViolation) {
            return response()->json(
                SafeErrorEnvelope::make('POS_INVENTORY_ACCOUNTABILITY_AUTHORIZATION_DENIED', $correlationId),
                403,
                ['Cache-Control' => 'no-store, private'],
            );
        } catch (PosTransactionViolation) {
            return response()->json(
                SafeErrorEnvelope::make('POS_INVENTORY_ACCOUNTABILITY_UNAVAILABLE', $correlationId),
                503,
                ['Cache-Control' => 'no-store, private'],
            );
        }
